...
import { Plus, Image as ImageIcon, GalleryHorizontal, Layers, LucideIcon, AlignHorizontalJustifyStart } from 'lucide-react';
...
export default async function AdminBannersPage() {
  ...
  const stats = {
    ...
    topBar: filteredBanners.filter(b => b.position === 'TOP_BAR').length,
    hero: filteredBanners.filter(b => b.position === 'MAIN_HERO').length,
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 lg:space-y-8 bg-white min-h-[calc(100vh-4rem)]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">
              Gestión de <span className="text-primary">Banners</span>
            </h1>
            <p className="text-sm sm:text-base text-slate-500 mt-1 sm:mt-2">
              Administra los banners promocionales de {storeName}.
            </p>
          </div>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard
          ...
          description="Barra de anuncios"
        />
        <StatCard
          title="Hero Principal"
          value={stats.hero}
          icon={GalleryHorizontal}
          description="Carrusel principal"
        />
      </div>
